...
| `assets/` | The page's own media. `glyph.svg` is the favicon and `apple-touch-icon.png` its home-screen counterpart; `fonts/` stays at the root because `crt.css` is copied into each project expecting it alongside |
| `index.css` | This page only: the operator, the tuner, the records |
| `assets/portrait.jpg` | The operator's plate, composited into whichever medium is on |
| `records.js` | The catalogue |
| `og.py` | Draws `assets/og.png`, the card the index unfurls as. Run by hand, not a build step |
| `assets/og.png` | 1200×630, committed. Regenerate it whenever the catalogue changes |
